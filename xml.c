...
static int get_app_attributes(ezxml_t app_attribute, appconfig_t * app_info)
{
	...
	/*Get appAttributes/sliding_window*/
	tmp = ezxml_child(app_attribute, "sliding_window");
	if (tmp == NULL) {
		printf("appAttributes->sliding_window is missing\n");
		return (-1);
	}
	...
	/*interpret and turn to boolean value*/
	if (strncmp(tmp->txt, "TRUE", 4) == 0){
		app_info->sliding_window = 1;
		app_info->window_width = (app_info->window_width)/5;
	}else if (strncmp(tmp->txt, "FALSE", 5) == 0){
		app_info->sliding_window = 0;
	}		
